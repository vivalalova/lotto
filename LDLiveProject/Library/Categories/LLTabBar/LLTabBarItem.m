...
- (instancetype)initWithFrame:(CGRect)frame withTabBarItem:(LLTabBarItemType)tabBarItem {
	self = [super initWithFrame:frame];
	if (self) {
        [self configwithTabBarItem:tabBarItem];
	}
...
- (void)configwithTabBarItem:(LLTabBarItemType)tabBarItem {
    if (tabBarItem == LLTabBarItemRise) {
        [self setImageEdgeInsets:UIEdgeInsetsMake(-5, 0, 5, 0)];
    }else{
        [self setImageEdgeInsets:UIEdgeInsetsMake(-2, 0, 2, 0)];
    }
