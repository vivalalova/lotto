...
#import "PhoneLiveAnchorPlayIsStop.h"
#import <Accelerate/Accelerate.h>

@interface PhoneLiveAnchorPlayIsStop ()
...
@implementation PhoneLiveAnchorPlayIsStop
...
- (void)awakeFromNib
{
    ...
    self.alpha = 0;
    [UIView beginAnimations:@"ShowAnchorPlayIsStopView" context:nil];
...
- (void)setUser_countNums:(NSString *)user_countNums
{
    _user_countLabel.text = [NSString stringWithFormat:@"%@人观看", user_countNums];
}
...
-(void)selectButtonWithTag:(NSInteger)tag{
    if ([self.delegate respondsToSelector:@selector(selectAnchorPlayIsStopButtonWithTag:)]) {
        [self.delegate selectAnchorPlayIsStopButtonWithTag:tag-100];
    }
}

- (IBAction)didSelectionAttentionButton:(id)sender {
    if ([self.delegate respondsToSelector:@selector(selectAnchorPlayIsStopAttentionButton:)]) {
        [self.delegate selectAnchorPlayIsStopAttentionButton:sender];
    }
}

- (IBAction)didSelectBackButton:(id)sender {
    if ([self.delegate respondsToSelector:@selector(selectAnchorPlayIsStopBackButton:)]) {
        [self.delegate selectAnchorPlayIsStopBackButton:sender];
    }
}

- (void)removeFromPhoneLiveVC
{
    [UIView beginAnimations:@"RemoveAnchorPlayIsStopView" context:nil];
    [UIView setAnimationDuration:1];
    self.alpha = 0;
