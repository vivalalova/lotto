...
@interface AttentionVC ()<UITableViewDelegate,UITableViewDataSource,SRRefreshDelegate,AttentionCellDelegate>
{
...
- (void)viewDidLoad {
    ...
    if (![NSString isBlankString:self.titleStr]) {
        [self setUsualNavigationBarWithBackAndTitleWithString:@"关注的人"];
        IsOwnBool = NO;
    }else{
        [self setUsualNavigationBarWithBackAndTitleWithString:@"关注的人"];
        IsOwnBool = YES;
...
{
    if (IsOwnBool) {
        [self sendURI_FocusRequestOFPage:@"1"];
    }else{
        [self sendOtherURI_FocusRequestOFPage:@"1"];
    }
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == currentPage*20 -1 && currentPage<=totalPage) {
        ...
        if (IsOwnBool) {
            [self sendURI_FocusRequestOFPage:pageNum];
        }else{
            [self sendOtherURI_FocusRequestOFPage:pageNum];
        }
    }
...
- (void)sendURI_FocusRequestOFPage:(NSString *)pageNum
{
...
- (void)sendOtherURI_FocusRequestOFPage:(NSString *)pageNum
{
    NSMutableDictionary *paramToken = [NSMutableDictionary dictionary];
    [paramToken setObject:User_Token forKey:@"token"];
    [paramToken setObject:pageNum forKey:@"page"];
    ...
    [AFRequestManager SafeGET:URI_attentionaesid parameters:paramToken success:^(NSURLSessionDataTask *task, id responseObject) {
        NSDictionary *result = responseObject;
        if ([result[@"status"] intValue] == 200) {
            _tableview.hidden = NO;
            currentPage = [result[@"data"][@"page"] integerValue];
