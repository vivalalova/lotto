...
-(instancetype)initWithFrame:(CGRect)frame imageID:(NSString*)imageId
{
    ...
    if (self) {
        UIImage *heartImage = [UIImage imageNamed:[NSString stringWithFormat:@"heart-%@",imageId]];
...
//    CGRect bounds = CGRectMake(0, 0, heartImageBorder.size.width/2, heartImageBorder.size.height/2);
//    UIRectFill(bounds);
...
//    UIGraphicsBeginImageContextWithOptions(size, NO, 0.0f);
////    [_fillColor setFill];
//    CGRect bounds1 = CGRectMake(0, 0, heartImage.size.width/2, heartImage.size.height/2);
//    UIRectFill(bounds1);
