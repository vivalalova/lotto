...
-(void)playAnimation:(NSString *)animationName withRange:(NSRange)range numberPadding:(int)padding ofType:(NSString *)ext fps:(NSInteger)fps repeat:(int)repeat completion:(void (^)())completionBlock
{
    ...
    NSString *bundlePath = [[NSBundle mainBundle].resourcePath stringByAppendingPathComponent:[NSString stringWithFormat:@"higherGift.bundle/%@",animationName]];
...
//        NSString *suffix = [NSString stringWithFormat:paddingFormat, i];
		NSString *filename = [NSString stringWithFormat:@"%@%d", animationName, i];
        NSString *path = [bundle pathForResource:filename ofType:ext];
        NSString *retinaFilename = [NSString stringWithFormat:@"%@.%@",filename,ext];
...
-(void)update:(CADisplayLink *)dl
{
    ...
    if(timeSinceLastAnimationFrame >= animationFrameDuration)
    {
        ...
        {
            [self stopAnimating];
            if (self.delegate && [self.delegate respondsToSelector:@selector(animationViewIsFinished)]) {
                [self.delegate animationViewIsFinished];
            }
