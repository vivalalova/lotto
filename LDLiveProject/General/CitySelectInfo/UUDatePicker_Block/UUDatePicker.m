...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.minLimitDate) {
        ...
        self.minLimitDate = [self dateFromString:@"194901010000" withFormat:@"yyyyMMddHHmm"];
        minDateModel = [[UUDatePicker_DateModel alloc]initWithDate:self.minLimitDate];
    }
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView
{
    ...
    if (self.datePickerStyle == UUDateStyle_YearMonthDay){
        if (isIOS7) {
            int aaa = 120*self.frame.size.width/320;
            int bbb = 190*self.frame.size.width/320;
            int ccc = 250*self.frame.size.width/320;
            [self creatValuePointXs:@[[NSString stringWithFormat:@"%d",aaa],[NSString stringWithFormat:@"%d",bbb],[NSString stringWithFormat:@"%d",ccc]]
                          withNames:@[@"年",@"月",@"日"]];
        }
        return 3;
    }
